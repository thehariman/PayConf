...
if (firebaseConfig.apiKey !== "YOUR_API_KEY") {
    ...
    // Ensure we are authenticated before starting listeners
    onAuthStateChanged(auth, (user) => {
        if (user) {
            ...
            if (sessionStorage.getItem('showLoginSuccess')) {
                // Short delay so the UI renders first before showing the toast
                setTimeout(() => {
...
function setupListeners() {
    if (uploadTrigger) uploadTrigger.addEventListener('click', () => excelInput.click());
    ...
    if (navDashboard) navDashboard.addEventListener('click', (e) => { e.preventDefault(); if(statusFilter) statusFilter.value = 'all'; setActiveNav(navDashboard); applyFilters(); });
    if (navUnverified) navUnverified.addEventListener('click', (e) => { e.preventDefault(); if(statusFilter) statusFilter.value = 'pending'; setActiveNav(navUnverified); applyFilters(); });
    if (navApproved) navApproved.addEventListener('click', (e) => { e.preventDefault(); if(statusFilter) statusFilter.value = 'approved'; setActiveNav(navApproved); applyFilters(); });
    if (navRefunded) navRefunded.addEventListener('click', (e) => { e.preventDefault(); if(statusFilter) statusFilter.value = 'refunded'; setActiveNav(navRefunded); applyFilters(); });
    ...
    // Native PWA installation handles itself automatically.

     // Stat Cards
    const appCard = document.querySelector('.stat-card.approved');
    if (appCard) appCard.addEventListener('click', () => { if(statusFilter) statusFilter.value = 'approved'; applyFilters(); });
    
    const penCard = document.querySelector('.stat-card.pending');
    if (penCard) penCard.addEventListener('click', () => { if(statusFilter) statusFilter.value = 'pending'; applyFilters(); });
    
    const refCard = document.querySelector('.stat-card.refunded');
    if (refCard) refCard.addEventListener('click', () => { if(statusFilter) statusFilter.value = 'refunded'; applyFilters(); });
}
...
function listenToTransactions() {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        if (error.code === 'permission-denied') {
            showToast("Database Permission Denied. Check Security Rules.", "error");
        }
    });
}
...
async function handleExcelUpload(e) {
    ...
    // UI: Show loading state on button too
    if (uploadTrigger) {
        uploadTrigger.disabled = true;
        uploadTrigger.innerHTML = '<i data-lucide="loader-2" class="spinner"></i><span>Uploading...</span>';
    }
    ...
    reader.onload = async (evt) => {
        ...
        else showToast("No valid transactions found.", "warning");
        ...
        if (uploadTrigger) {
            ...
            uploadTrigger.innerHTML = '<i data-lucide="upload-cloud"></i><span>Upload CSV</span>';
            lucide.createIcons();
        }
        if (excelInput) excelInput.value = ''; // Reset input to allow re-upload
    };
...
function updateStats() {
    ...
    if (totalAmountEl) totalAmountEl.innerText = `₹${stats.total.toLocaleString()}`;
    if (approvedAmountEl) approvedAmountEl.innerText = `₹${stats.approved.toLocaleString()}`;
    if (pendingAmountEl) pendingAmountEl.innerText = `₹${stats.pending.toLocaleString()}`;
    if (refundedAmountEl) refundedAmountEl.innerText = `₹${stats.refunded.toLocaleString()}`;
    if (approvedCountEl) approvedCountEl.innerText = stats.approvedCount;
    if (pendingCountEl) pendingCountEl.innerText = stats.pendingCount;
    if (refundedCountEl) refundedCountEl.innerText = stats.refundedCount;
}
