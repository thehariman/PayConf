...
function listenToMappings() {
    onSnapshot(collection(db, "payment_mappings"), (snapshot) => {
        ...
        if (noteFilter) {
            // First, clear everything to prevent duplicates
            noteFilter.innerHTML = ""; 
            
            // Add the single correct "All" option
...
async function handleExcelUpload(e) {
    ...
    reader.onload = async (evt) => {
        ...
        for (const item of rawData) {
            ...
            if (!finalDate) {
                console.error("Missing or invalid date for transaction:", txId);
                skipCount++;
                continue; // Skip if no valid date in sheet
            }
        ...
        if (newCount > 0) {
            showToast(`Upload complete! ${newCount} new added, ${skipCount} duplicates skipped.`, "success");
        } else if (skipCount > 0) {
            showToast(`All ${skipCount} transactions were duplicates and were skipped.`, "warning");
        } else {
            showToast("No new data found in the sheet.", "error");
        }
    };
...
function renderTable() {
    ...
    filteredTransactions.map(tx => {
        const date = new Date(tx.creationTime);
            ...
            <tr>
                ...
                <td>
                    <div style="display: flex; align-items: center; gap: 8px;">
                        ...
                        <button onclick="copyTxId('${tx.transactionId}')" class="icon-btn-small" title="Copy ID" style="padding: 2px;">
